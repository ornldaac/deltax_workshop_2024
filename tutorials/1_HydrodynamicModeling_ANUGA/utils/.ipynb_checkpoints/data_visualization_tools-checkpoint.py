...
def x_corr(model_results, gauge_measurements, max_lag = 200, discard = 50):
    wl_df = pd.DataFrame({'model': model_results, 'gauge': gauge_measurements})
    wl_df = wl_df.iloc[discard:,:]
    max_lag = np.min([max_lag, wl_df.shape[0]])
    roll_results = pd.DataFrame({'RMSE': np.zeros(max_lag), 'R': np.zeros(max_lag), 'bias': np.zeros(max_lag)})
    for nn in range(0, max_lag):
        if nn>0:
            wl_df['gauge'] = wl_df['gauge'].shift(periods=1)
        wl_diff = wl_df['model']-wl_df['gauge']
        roll_results.loc[nn, 'RMSE'] = np.nanmean(np.sqrt(wl_diff)**2)
        roll_results.loc[nn, 'R'] = wl_df.corr().loc['model', 'gauge']
        roll_results.loc[nn, 'bias'] = np.nanmean(wl_diff)
    if roll_results['RMSE'].dropna().shape[0] > 0:
        minRMSE_lag = roll_results['RMSE'].where(roll_results['RMSE'] == roll_results['RMSE'].min()).dropna().index.values[0]
    else:
        minRMSE_lag = np.nan
    if roll_results['R'].dropna().shape[0] > 0:
        maxR2_lag = roll_results['R'].where(roll_results['R']**2 == (roll_results['R']**2).max()).dropna().index.values[0]
        maxR_lag = roll_results['R'].where(roll_results['R'] == roll_results['R'].max()).dropna().index.values[0]
    else:
        maxR2_lag = np.nan
        maxR_lag = np.nan
    
    return roll_results['R'].to_numpy(), roll_results['RMSE'].to_numpy(), maxR_lag, maxR2_lag, minRMSE_lag, roll_results['bias'].to_numpy()
...
def animate_stage(saved_output, domain_polygon, gauge_location_ind = None, gauge_time = None, gauge_stage = None, bg_img_filepath = None, fps = 5, initial_frame = 0, final_frame = 1, framestep = 1, dpi = 150, save = False, filename = 'stage.mp4'):
    ...
    if gauge_location_ind is not None:
        axes[0].scatter(np.nanmean(x[gauge_location_ind]), np.nanmean(y[gauge_location_ind]), marker='x', color='w', s=50)
...
def animate_velocity(saved_output, domain_polygon, querry_xy_inds, gauge_location_ind = None, gauge_time = None, gauge_stage = None, bg_img_filepath = None, fps = 5, initial_frame = 0, final_frame = 1, framestep = 1, dpi = 150, save = False, filename = 'velocity.mp4'):
    ...
    subfig1 = axes[0].tripcolor(triang, np.mean(o[triangles], axis=1), antialiased=True, 
             cmap='bwr', vmin=0.01, vmax=.3, alpha=.5, edgecolors='face', linewidths=0)
    ...
    Quiv = axes[0].quiver(np.nanmean(x[querry_xy_inds], axis=1), 
                          ...
                          scale=50, width=0.005, headwidth=5, headlength=7, headaxislength=5,
                          pivot='tail', alpha=.7)
